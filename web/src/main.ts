...
import { avatarFileError, safeAvatarSource } from './avatar'
...
function avatarView(account: Pick<AccountProfile, 'display_name' | 'username'> & { avatar?: string }, size = 'small') {
  ...
  const source = safeAvatarSource(account.avatar)
  return `<span class="avatar avatar-${size}">${source ? `<img src="${escape(source)}" alt="">` : escape(letters)}</span>`
}
...
async function hydrateWorkspace() {
  try {
    const [imports, trades, account, portfolio] = await Promise.all([
      ...
      withAuth((accessToken) => api.ownProfile(accessToken)).catch(() => null),
...
async function searchAccounts(query: string) {
  ...
  try {
    ...
    if (results) {
      ...
      results.setAttribute('aria-label', state.accountResults.length ? `${state.accountResults.length} public account results` : 'No public account results')
